...
import unicodecsv
print( json.load(requests.get("http://fortune.com/fortune500/walmart/"), encoding='utf-8'))
...
def grab():
    ...
    # Obtaining post id
    data = requests.get("http://fortune.com/fortune500")
    soup = BeautifulSoup(data.text ,'html.parser')
    print(soup)
    print(soup.div['class'])

    postid = next(attr for attr in soup.li['class'] if attr.startswith('postid'))
    ...
    for i in range(1,51):
        ...
        page_data = json.load(urllib.urlopen(page_url), encoding='utf-8')
